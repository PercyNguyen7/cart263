// STATES
//  loading function to give advice to players
function loading() {
  background(40);
  displayText(`"To help or not to help... `, 30, 2 * width / 5, height / 6, 255);
  displayText(`That is the question"`, 30, 2 * width / 3, height / 4, 255);
  displayText(`by Don't fix-it felix!`, 30, 2 * width / 3, 3 * height / 4, 255);
    displayText(`and Percy Nguyen`, 30, 2 * width / 3, 3.3 * height / 4, 255);
}
...
// Instruction state as a test trial for player to test to be able to begin.
...
// Introduction state gives context to the game
...
// First situation state where Player meets Michael and M drops phone, prompting decision 1
...
// Intro of first Decision to explain to players how to play
...
// Player may choose to let phone drop or catch it. Catch by saying "I got you" and putting fingertips near the phone.
...
// If the phone reaches the bottom and player fails to catch, display this state. M juggles phone with left feet. Both are happy
...
// Achieved by saying I Got You while having all 5 fingers on the phone. Player breaks phone and M is angry.
...
// Second situation where a bird poops on Michael.
...
// Intro to second Decision to explain to players.
...
// Player may chooses to Push M away from poop or not. To push, P says "Poop from the sky" while moves hand from left half to right half of the screen.
...
// Do nothing outcome if player let poop falls. Wind pushes it away and Michael ties his shoes.
function doNothing2Outcome() {
  ...
  if (eventCounterDN2 === 0) {
    ...
    displayText(`He looks at you with shiny eyes`, 25, 20, 4.3 * height / 5, 70, 70, 70);
...
//Third situation where a car runs at Michael as he is picking up the soccer ball.
function thirdSituation() {
  ...
  if (eventCounterS3 === 2) {
    displayText(`Something seems wrong...`, 25, 20, 4.3 * height / 5, 70, 70, 70);
  } else if (eventCounterS3 === 3) {
    displayText(`Something seems wrong...`, 25, 20, 4.3 * height / 5, 70, 70, 70);
    displayText(`You look to the left to see...`, 25, 20, 4.7 * height / 5, 70, 70, 70);
  } else if (eventCounterS3 === 4) {
    s3bg = situation3bg2Image;
    displayText(`A CAR `, 55, 20, 4.3 * height / 5, 70, 70, 70);
  } else if (eventCounterS3 === 5) {
    s3bg = situation3bg3Image;
    displayText(`RUNNING TOWARDS HIM AT FULL SPEED!`, 44, 20, 4.3 * height / 5, 70, 70, 70);
...
// Intro to Third Decision
function thirdDecisionIntro() {
  background(160, 0, 0);
  push();
  displayText(`WILL YOU`, 45, width / 2, height / 2 - 150, 255);
  displayText(`SAVE MICHAEL by pushing him to the NEXT LANE? (kinda cliche)`, 30, width / 2, height / 2 - 100, 255);
...
// Player may does nothing or pushes Michael away by yelling "Not on my watch" and shows P's middle finger of at least half the size of the canvas.
...
// If player chooses to push M to next lane.
...
// Last Decision. Player may presses A to call 911 or B to leave the scene
...
function loopEnding() {
  ...
  // 0 help would be impossible as they did push Michael to achieve this Ending// 0 help would be impossible as they did push Michael to achieve this Ending
...
function atonedEnding() {
  ...
  // 0 help would be impossible as they did push Michael to achieve this Ending
  else if (helpCounter === 0) {
    displayText(`Helped 0 time`, 30, width / 2, height / 2 + 100, 255, 255, 255);
    displayText(`OI stop bloody hacking mate!`, 30, width / 2, height / 2 + 140, 255, 255, 255);
  }
...
// Futile Ending achieved if player chose NOT to save Michael from the incoming car.
