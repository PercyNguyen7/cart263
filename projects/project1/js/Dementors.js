...
class Dementors {
  constructor(dementorImage, dementor2Image, dementor3Image) {
    // Propertiese of first Dementor (biggest on left)
    this.x = -300,
      this.y = height + 100,
      this.vx = 12;
    this.vy = -6;
    this.width = 300,
      ...
      this.levitate = false;
    // Propertiese of second Dementor (medium size on right)
    this.x2 = width + 500,
      this.y2 = -50,
      this.vx2 = -12;
    this.vy2 = 5.5;
    ...
    // Propertiese of third dementor (smallest in the back);
    this.x3 = 3 * width / 4,
      this.y3 = -700,
      this.vx3 = -2.67;
    this.vy3 = 12;
  ...
  move() {
    ...
    // }
    // If levitate is true then replace vx and vy with levitate speed so they slowly levitate around randomly.
    // Size is also multiplied and increased gradually
  ...
  disappear() {
    ...
    // Lose if first dementor's width reaches 1200
    if (this.width === 1200) {
      state = `dementorEnding`;
      dementorEndingSFX.play();
      dementorsSFX.stop();
      stage123Soundtrack.stop();
      warSFX.stop();
      expectopatronusSFX.stop();
